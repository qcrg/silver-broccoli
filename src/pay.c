...
rtp_pkt_t *rtp_pay_pull(rtp_pay_ctx_t *ctx)
{
    ...
    rtp_pkt_t *pkt;
    int remainder = ctx->sample->payload_size - (data->size - ctx->data_offset);
    if (remainder <= 0) {
        pkt = ctx->sample;
    } else {
        uint_ size = ctx->sample->payload_size - remainder;
        if (small_pkt) {
            if (small_pkt->payload_size != size) {
                ctx->dealloc(small_pkt);
    ...
    memcpy(pkt->payload_begin, data->data, pkt->payload_size);
